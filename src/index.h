...
<body style="text-align: left;">
  <hr><b><i>Current setttings are sent from the MCU every 7 seconds.</i>
    <table class="calibrate">
      <caption>Set Temp & Humidty Bounds</caption>
        ...
        <tbody>
            <tr>
                <td><input type="number" id="t1_on"  name="limit" min="60" max="87" ></td>
                <td><input type="number" id="t1_off" name="limit" min="60" max="87"></td>
                <td><input type="number" id="h1_on"  name="limit" min="60" max="87" ></td>
                <td><input type="number" id="h1_off" name="limit" min="60" max="90"></td>
                <td><input type="number" id="t2_on"  name="limit" min="60" max="87" ></td>
                <td><input type="number" id="t2_off" name="limit" min="60" max="87" ></td>
                <td><input type="number" id="delay"  name="limit" min="2"  max="999" ></td>
                <td></td>
            </tr>
        </tbody>
    </table>
    <br></b>Current MCU Settings<p id="settings"></p>
  </body>
  <script language = "javascript" type = "text/javascript">
      ...
      function f_webSockOnMessage(evt){
        if(typeof evt.data === "string"){
          j=JSON.parse(evt.data);
          document.getElementById("settings").innerHTML=evt.data;
            for(var key in j[1]){
...
<html lang="en">
  ...
  <body style="text-align: left;">
    <hr>
      <table class="calibrate">
        <caption>Set Temp & Humidty Relays SSID : PWD</caption>
          <thead>
              <tr>
                  <th>SSID</th>
                  <th>PWD</th>
    ...
    <script language = "javascript" type = "text/javascript">
        ...
        function f_webSockOnError(evt){}
        function f_submit(){
          var str= '{"SSID":"0","PWD":"0"}';
          j=JSON.parse(str);
            for(var key in j)j[key]=document.getElementById(key).value;
